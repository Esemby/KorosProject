...
if (naming == 2)
{
    if (charname == "")
    {
        spec_m = scr_gettext("name_entry_missing"); // You must choose a name.
        allow = 0;
    }
    else if (scr_hardmodename(charname))
    {
        spec_m = scr_gettext("name_entry_hardmode"); // WARNING: This name will#make your life hell.#Proceed anyway?
        allow = 1;
    }
    else if (hasname == 1 && truereset == 0 && !scr_hardmodename(global.charname))
    {
        spec_m = scr_gettext("name_entry_already"); // A name has already#been chosen.
        allow = 1;
    }
    else
    {
    ...
    if (allow)
    {
        ...
        scr_drawtext_centered(80, 200, scr_gettext("no")); // No
        draw_set_color(c_white);
        
        if (selected2 == 1)
            draw_set_color(c_yellow);
        
        scr_drawtext_centered(240, 200, scr_gettext("yes")); // Yes
    }
    else
    {
        draw_set_color(c_yellow);
        scr_drawtext_centered(80, 200, scr_gettext("name_entry_goback")); // Go back
        draw_set_color(c_white);
    }
...
if (naming == 1)
{
    ...
    var menu_text0 = scr_gettext("name_entry_quit"); // Quit
    ...
    var menu_text1 = scr_gettext("name_entry_backspace"); // Backspace
    ...
    var menu_text2 = scr_gettext("name_entry_done"); // Done
    ...
    scr_drawtext_centered(160, title_y, scr_gettext("name_entry_title")); // Name the fallen human.
}
...
if (naming == 3)
{
    ...
    if (ini_section_exists("General") && hasname == 1)
    {
        ...
        var continue_text = scr_gettext("load_menu_continue"); // Continue
        ...
        scr_drawtext_centered(160, 125, scr_gettext("settings_name")); // Settings
        draw_set_color(c_white);
        ...
        if (truereset == 0)
            reset_text = scr_gettext("load_menu_reset"); // Reset
        else
            reset_text = scr_gettext("load_menu_truereset"); // True Reset
    ...
    else
    {
        ...
        if (global.osflavor >= 4)
        {
            ...
            draw_text(115, 50, scr_gettext("instructions_confirm_label")); // Confirm
            scr_drawtext_icons(85, 70, "\*X");
            draw_text(115, 70, scr_gettext("instructions_cancel_label")); // Cancel
            scr_drawtext_icons(85, 90, "\*C");
            draw_text(115, 90, scr_gettext("instructions_menu_label")); // Menu (In-game)
            draw_text(86, 130, scr_gettext("instructions_hp0")); // When HP is 0, you lose.
        }
        else
        {
            var lines;
            ...
            if (global.language == "ja")
            {
                ...
                {
                    var key = scr_gettext("instructions_" + lines[i] + "_key"); // <localization fetch error>
                ...
                for (var i = 0; i < num_lines; i++)
                {
                    var label = scr_gettext("instructions_" + lines[i] + "_label"); // <localization fetch error>
                    draw_text(label_x, 45 + (i * 18), label);
                }
                
                draw_text(50, 145, scr_gettext("instructions_hp0")); // When HP is 0, you lose.
            }
            else
            {
                for (var i = 0; i < num_lines; i++)
                ...
                draw_text(85, 140, scr_gettext("instructions_hp0")); // When HP is 0, you lose.
            }
        }
        ...
        draw_text(xx, yy, scr_gettext("instructions_begin")); // Begin Game
        ...
        draw_text(xx, yy2, scr_gettext("settings_name")); // Settings
        var action = -1;
